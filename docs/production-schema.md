<!--
  Carried from interlock at 65f36c5 (docs/production-schema.md).

  Provenance, and which copy is live:

  - **Origin.** interlock's `docs/production-schema.md`, at revision `65f36c5` -- the revision
    continuo's parity ledgers and oracle vectors are pinned to.
  - **This copy is the live one.** From this commit on, continuo's code, SQL and tests cite *this*
    file, and a decision that changes a number here is made here. The copy in interlock is not
