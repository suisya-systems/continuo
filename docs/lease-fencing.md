<!--
  Carried from interlock at 65f36c5 (docs/lease-fencing.md).

  Provenance, and which copy is live:

  - **Origin.** interlock's `docs/lease-fencing.md`, at revision `65f36c5` -- the revision continuo's
    parity ledgers and oracle vectors are pinned to.
  - **This copy is the live one.** From this commit on, continuo's code and tests cite *this* file,
    and a decision that changes the destination register is made here. `test_lease.py`'s
    `every registered destination is written down in the doc` reads the register table below and
    compares it, by set equality, against `DESTINATIONS` in `src/control_plane/lease.ts` -- so a
    destination added to the code and not to this table is a red test, which is the whole point of
    the table existing.
  - **The lineage of record is still interlock.** Per interlock#74, a `D-00NN` cited below without
    qualification is an **interlock** decision id, not one of continuo's `DECISIONS.md` entries.
  - **Language-specific notes** may be adjusted here; the register, the numbers and the decisions
    may not -- those are what `parity/` asserts against.
-->
