...
D-0026 (spike output is throwaway by default), D-0001 (state reconstructable by query); `Q-0005`
(unresolved — canary duration, sample size, numeric go/no-go, and the treatment of runs in flight
*on Interlock* at rollback). The D-numbers in this document are interlock's design record, which
this port carries over unchanged; continuo's own decisions about the port of this belt are recorded
in continuo's `DECISIONS.md` under the D-04xx band.
...
caller takes the answer to the owning system's own start path. It imports no other module of this
package (not `session`, not `dispatcher`, not `control_plane`), which is asserted on the syntax
tree by `test/canary/structural.test.ts`: the routing point has no provider dependency and
survives C2 — or any later provider switch — untouched.

The one exception the port carries, and the reason it is not a widening of the claim: `ledger.ts`
and `routing.ts` import `src/sqlite/errors.ts` for `isSqliteError` / `sqliteCodeOf`. Python's
`sqlite3` exception classes are *built in*, so interlock's canary needed no import to classify a
store failure by result code; TypeScript's driver surfaces the same result codes through a helper
module, and classifying by message text instead is forbidden (D-0016). The allowance is exactly
that one module, named in the structural test rather than granted as a pattern, so anything else
reaching out of `src/canary/` still fails there.
...
That the store is what enforces this, rather than the code above it, is load-bearing enough that
the port pins one thing the source got for free: every writable connection this package hands out
is configured with `recursive_triggers = ON` as well as `foreign_keys = ON`. With recursive
triggers off — SQLite's default — `INSERT OR REPLACE` resolves a primary-key conflict with an
...
therefore defined over three facts of the construction (`src/canary/audit.ts`):
...
The exclusion is by *table name* only: the `routing_decision` schema line stays in the digest even
when its rows are held out, so a trigger quietly dropped from the append-only relation is still a
difference the comparison sees.

## What the rehearsal shows — and what it cannot

`test/canary/rehearsal.test.ts` plays the canary shape (D-0013) against the stand-in with three
