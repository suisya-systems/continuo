...
are writes a report tool must never trigger. So the harness opens the database with the read-only
open flag -- interlock passes the SQLite `mode=ro` URI; better-sqlite3 does not accept URI filenames
at all, so continuo passes `readonly: true` to the driver instead, which sets the same
`SQLITE_OPEN_READONLY` the URI asked for (`D-0100`) -- **and** `PRAGMA query_only=ON`, and it never
calls `migrateControlPlane` (`production-schema.md` section 3.2 rule 5 keeps migration a separate,
explicit call precisely so this is possible).

`ACCEPTANCE.md` section 3 condition 5 requires the shadow path to be read-only **enforced by
capability, not by convention**. Two connections, two enforcements: the mode is the capability, and
the harness holds no lease and no writer epoch, so even a bug cannot produce a fenced write.
...
2. **Ownership is decided once, at run start** (interlock `D-0013`, `ACCEPTANCE.md` section 3
   conditions 3 and 4), so "Interlock-owned for its entire life" is not an extra filter -- it is
   automatic, and it is stated only so that the report can assert it rather than assume it.
...
tokens are not output tokens and are not input tokens**; `ACCEPTANCE.md` section 5 says so explicitly
...
| **Bounded reduction** | Missing invocations imputed at **`max_output_tokens x model_response_count`** -- the caller's own per-request output cap, times the number of responses the invocation made | **A genuine lower bound on the reduction** |
...
the caller allowed, so imputing a missing invocation at `max_output_tokens x model_response_count`
...
and the imputation rule is recorded in the report header (section 6) so a reader can recompute under a
different one.
...
| `onset_offset_ms` | When the condition **began** -- the state entry -- relative to `t0`. Not the tolerance crossing: `T` is part of `L`, not a head start on it (`time-base-policy.md` section 3.1), and labelling the crossing here would hand every fixture an extra `T` of slack and pass alarms that landed at `T + L` |
...
| `fact_state` | The interlock `D-0005` state the detector should classify to |
...
- **A false termination** is a fixture where a recommendation in `must_not_recommend` was produced
  *and applied* -- see section 3.4.

**Negative cases are mandatory, not optional.** Interlock `D-0006` requires observation-failure
fixtures alongside stall fixtures, and a corpus of only positive cases would let a detector that
alarms on everything score a perfect miss rate. The suite's composition is reported (section 6) so
that a recall improvement bought by widening every predicate shows up as a false-positive regression
in the same table.
...
| `censored` | The episode's observation window extends beyond the report period (section 3.5) |
...
The review's point is decisive: interlock `D-0004` and AC-6 mean the Dispatcher AI cannot terminate
anything. Counting AI recommendations, or watcher candidates, would compare Interlock's
recommendations against v1's *executions* -- and counting Interlock's executions of a capability it
does not have would report a structural zero as a triumph.
...
episode is `undetermined` and appears in its own bucket -- interlock `D-0006`'s "cannot determine is
a legitimate outcome" applied to the measurement instead of to the detection.
...
`time-base-policy.md` section 2 rule 3) is printed beside it, so that a
latency regression caused by a slow provider is separable from one caused by us. Without it, GitHub
having a bad afternoon reads as a detection regression.
...
report is section 3.3's episode reconciliation rendered per period, plus:

- **A writer audit** across both stores over the canary window, asserting no record was written by
  both (`ACCEPTANCE.md` section 3 condition 2, verification bullet 1).
...
transaction** (`measurement/reader.measurementSnapshot`) that covers every read *including* the
...
in WAL (section 3 of `production-schema.md`; `createProductionControlPlane` leaves `journal_mode` at
...
- **The escalation correlation key is positional** (section 3.3) and is the weakest join in the
  ...
  anything.
- **`ai_invocation` is new state** and is named in interlock `D-0029`'s entity list extension; it is
  written by the component that invokes the Dispatcher AI, which is a single writer by construction
  (the AI is on-demand and incident-triggered -- interlock `D-0003`).
