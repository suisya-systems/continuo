...
**One index is not enough, because P-6 leaves `due` with two call shapes.** The recipient term is
optional (P-6): `MessageBus.poll` supplies it, and every other caller -- the fault-injection belt and
the existing assertions -- keeps calling `due(nowMs)` with no recipient at all. On that second shape
the composite index cannot seek on `enqueued_at_ms`, because the `recipient` column sitting between
the two constrained ones is unconstrained. The call then visits **every unfinished row of the
resource**, future-dated rows included, and sorts them in a temporary B-tree for the `ORDER BY`. That
is a regression against `outbox_undelivered`, which serves that shape today.

So the change carries **two** indexes -- `(delivery_resource, recipient, enqueued_at_ms)` and
`(delivery_resource, enqueued_at_ms)`, both with the partial predicate -- and the entry says why the
second is not redundant rather than leaving a later reader to drop it as duplication.

**And the plan assertion has to be sharpened, or it cannot see this.** The existing test asserts
`SEARCH` plus the index name (`outbox.test.ts:2769-2807`). Both are still true of the degraded shape:
SQLite reports `SEARCH` for the `delivery_resource` equality alone and names the composite index while
using only its first column. **An assertion of that form would pass on exactly the regression this
paragraph describes.** The evidence must therefore check that the **range term itself** is being used
-- the plan text carries the constraint count (`SEARCH outbox USING INDEX ... (delivery_resource=? AND
recipient=? AND enqueued_at_ms>?)` against `(delivery_resource=?)`) -- and it must be taken for
**both** call shapes, each with its own degraded twin. This is the same lesson as section 9.3's masked
controls arriving in the query planner: a check that passes for the wrong reason is worth less than no
...
| **P-9** | Use the next forward migration (`0005`); never edit a historical one; backfill existing rows to the exact literal `"outbox-delivery"` **and mark them as inherited**, since the rebuild is rewriting every row anyway and that marker is the only sound provenance for P-16's legacy ack exception (a clock comparison is not: both timestamps are caller-supplied and unordered); replace the due index with **two** measured partial forms -- `(delivery_resource, recipient, enqueued_at_ms)` and `(delivery_resource, enqueued_at_ms)` -- because P-6 leaves `due` with two call shapes and the composite index cannot seek the range term when the intervening `recipient` is unconstrained. Keep positive **and** degraded EXPLAIN evidence for **each** shape, and assert on the **constraint list** rather than on `SEARCH` plus an index name, since both remain true of the degraded plan (section 5.4). Say explicitly in the entry that `sqlite-value-contract.md` is a value contract and not a schema freeze. **Prefer the 12-step rebuild over `ADD COLUMN ... NOT NULL DEFAULT`** (section 5.2); the gate may take the default instead with a schema test pinning its legacy-only meaning. | pre-review, amended |
...
**Rounds A1-A2 -- a second reviewer.** Rounds 1-10 were all one model (`gpt-5.6-sol`). A1 and A2 are
two passes by a different one (`gpt-6-astra`), asked to look at the whole change with the ten previous
rounds visible in this appendix. Each raised **one** finding, neither a repeat of anything above, and
both landed on the same seam: a repair made in a late round that was never followed into the section
it affected. A1 accepts round 10's *problem statement* and rejects its *solution*; A2 finds round 8's
optional-recipient repair unpaid-for in the query plan.

| # | Finding | Verdict | Where answered |
|---|---|---|---|
| A2 | The replacement index regresses the recipient-less `due`: with `recipient` unconstrained between the two constrained columns, the composite index cannot seek `enqueued_at_ms`, so that call visits every unfinished row of the resource and sorts in a temporary B-tree -- and a `SEARCH`-plus-index-name assertion still passes, because SQLite reports both while using only the first column | **Confirmed, and it is round 8's repair casting a shadow into section 5.4.** Making `recipient` optional (P-6) gave `due` two call shapes; section 5.4 had costed only one. The second half is the sharper half: the proposed evidence would have passed on exactly this regression. | Section 5.4 carries **two** partial indexes and says why the second is not redundant; the plan evidence is sharpened to assert the **constraint list** rather than `SEARCH` plus a name, for both shapes with their own degraded twins; **P-9 amended** |
...
**The twenty-seven fall into six groups, and the order they arrived in is itself the finding.** Rounds
