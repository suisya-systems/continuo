...
**The admission has to be bounded in time, or it weakens the check for every future row.** As stated
above the rule is permanent, and that is wrong: a *post*-migration run-bound relay stamped with the
global resource is precisely the corruption the paragraph before it says to refuse, and a permanent
two-value rule would accept it and advance the gate. The exception exists only for rows a migration
inherited, so it must be scoped to them -- and the boundary is already durable in the database.
`schema_migration` carries `applied_at_ms` per version and is protected against both deletion and
update by triggers (`migrator.ts:93-112`), so the admissible form is:

> the global literal is admitted only when the row's `enqueued_at_ms` is at or before `0005`'s
> `applied_at_ms`, read from `schema_migration`.

A run-bound relay on the global resource enqueued **after** that instant is refused as corruption, which
is what the rule was supposed to say. This costs one subquery against an immutable table and no schema
change, and it is why the boundary is worth reaching for rather than approximating: provenance that a
trigger protects is provenance an implementation cannot quietly lose.

Admitting the literal for the rows below that boundary costs nothing this check was defending. The attack P-16 exists to stop is a
...
**The twenty-five fall into six groups, and the order they arrived in is itself the finding.** Rounds
