 ...
 *
 * Eleven verbs answer in it; a twelfth, `measure report`, mounts the same flag
 * and emits its report UNWRAPPED, for the reason `measurement/cli.ts` gives.
 * The counts below are of the twelve that mount the flag, because what this
 * module exists to prevent is twelve spellings of one flag.
 *
 ...
 * twelve verbs must spell one flag identically, and twelve independent
 * `addArgument` calls are twelve chances to disagree about the help text, the
 ...
 * `--help` on a cp932 console from all twelve verbs at once.
 */
...
/** The flag's help text, declared once so twelve verbs cannot disagree about it. */
