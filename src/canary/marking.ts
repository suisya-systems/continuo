 ...
 *
 * Written as a concatenation of the source's four physical lines rather than as
 * one long literal, so that a diff against `marking.py` lines up and a
 * re-wrapping cannot silently change a space into a newline. The value is a
 * single line with single spaces -- `routing_ledger.sql` carries it wrapped
 * across four comment lines, and `collapsedLedgerSql` is what makes the two
 * comparable.
 */
export const REHEARSAL_MARKING =
